...
public class POOBkemon implements Serializable {
    ...
    private void validateInitialData(ArrayList<String> trainers, HashMap<String, ArrayList<Integer>> pokemons, HashMap<String, String[][]> items) throws POOBkemonException {
        if (trainers == null || trainers.isEmpty()) throw new POOBkemonException(POOBkemonException.MISSING_TRAINER_DATA);
        if (pokemons == null || pokemons.isEmpty()) throw new POOBkemonException(POOBkemonException.MISSING_POKEMON_DATA);
	 ...
	 * @param itemsData informacion del item.
	 ...
	 * @param pokemonsIds los ids de los pokemones
	 * @param attacksId los ids de los ataques que tendra el Pokemon.
	...
	public void attack(int idAttack, int idTrainer, int idThrower) throws POOBkemonException {
		...
		if (!attacker.getActive()){throw new POOBkemonException(POOBkemonException.POKEMON_INACTIVE);
		}
