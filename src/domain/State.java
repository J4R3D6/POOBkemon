...
public class State implements Serializable {
    ...
    public State(String[] info) {
        if (info == null || info.length < 5) {
            throw new IllegalArgumentException(POOBkemonException.INCOMPLETE_DATA);
        }
        try {
            ...
            throw new IllegalArgumentException(POOBkemonException.STATE_ERROR + e.getMessage());
        }
    }
    ...
    private boolean isHeal(State state, Pokemon pokemon){
        if (state.duration > 0) {
            duration--;
            if (duration == 0) {
                pokemon.deletePricipalState(this);
                return true;
            }
        }
        return false;
    ...
    private void applyParalysisEffect(Pokemon pokemon, StringBuilder message) {
        pokemon.modifyStat("speed",0.5); // Reduce velocidad en 50%
        if (Math.random() < 0.25) {
    ...
    public void applyTo(Pokemon target) {
        if (this.isPrincipal() && target.getPrincipalState() == null) {
            target.setPrincipalState(this);
        } else if (!this.isPrincipal()) {
            target.setSecundaryState(this);
        }
    }
